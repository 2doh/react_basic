...
const Average = () => {
  ...
  // useMemo를 사용할 때
  // input 내용이 바뀔 때는 평균 값을 계산할 필요 없음.
...
- 컴포넌트가 리렌더링 될 때 마다 새로 만들어진 함수를 사용하게 되는 것은 렌더링이 자주 발생하거나 렌더링해야할 컴포넌트의 개수가 많아지면 최적화 하는 것이 좋다.
...
- 여기서 로컬변수는 렌더링과 상관없이 바뀔 수 있는 **값**을 의미
- 실습예제 : 더블클릭 방지 기능 구현, 사용자가 버튼을 빠르게 여러번 클릭하는 경우 예상치 못한 여러 번의 액션 방지
